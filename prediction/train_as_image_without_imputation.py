...
from dataset.dataset_prediction_as_image import PRE8dDataset
...
parser.add_argument('--method', type=str, default='PredRNN', help='which bay area we focus')
...
assert config.method in ["PredRNN"], print("{} not implement".format(config.method))
if config.method == "PredRNN":
...
for epoch in train_process:
    ...
    for train_step, (datas, data_ob_masks, data_gt_masks, labels, label_masks) in enumerate(train_dloader):
        ...
        means = (datas.sum(1, keepdim=True)/(data_ob_masks.sum(1,keepdim=True)+1e-10)).detach()
        datas = datas - means
