...
from model.cross_models.cross_encoder import Encoder
from model.cross_models.cross_decoder import Decoder
from model.cross_models.attn import FullAttention, AttentionLayer, TwoStageAttentionLayer
from model.cross_models.cross_embed import DSW_embedding

from math import ceil

class Crossformer(nn.Module):
    def __init__(self, data_dim, in_len, out_len, seg_len, win_size = 4,
                factor=10, d_model = 32, d_ff = 64, n_heads=1, e_layers=1, 
    ...
    def forward(self, x_seq):
        batch_size , t, c =  x_seq.size()

        assert t==self.in_len, print("input seq length {} do not equal to pre defined seq length {}".format(t, self.in_len))
        ...
        prediction = base + predict_y[:, :self.out_len, :]
        return prediction
